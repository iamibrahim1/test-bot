let handler = async(m, {conn, command, usedPrefix, text}) => {
  ...
  let i = 0
  if (global.db.data.users[m.sender].catatan.length == 0) return m.reply('You dont have any notes yet!')
  ...
  if (text.length == 0) return await conn.sendButtonLoc(m.chat, 'https://telegra.ph/file/15e31900512863624ed57.jpg', txt, 'Use:\n' + usedPrefix + 'seenote 1\Delete record:\n' + usedPrefix + 'hapuscatatan 1', 'Okey', 'Ok', m)
  let catatan = global.db.data.users[m.sender].catatan
  let split = text.split('|')
  if (catatan.length == 0) return m.reply('You dont have any notes yet!')
...
handler.help = ['seenote']
handler.tags = ['tools']
handler.command = /^seenote$/i
